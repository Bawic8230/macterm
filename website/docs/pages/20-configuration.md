...
- **Smooth scrolling** — trackpad scrolling moves scrollback by pixels instead of whole rows, including during momentum, and dragging the scrollbar does the same instead of jumping a row at a time. Resizing a pane moves it by pixels as well, so dragging a split divider (or an animated split) slides the text rather than stepping it a line at a time; a pane whose height isn't a whole number of lines shows the line above it partly revealed at the top. Only scrollback scrolls this way: programs that draw their own screen (editors, pagers, `less`) still scroll by rows, because that is how they repaint. Mouse wheels keep row-by-row scrolling, and `mouse-scroll-multiplier` tunes the speed of both as in Ghostty.
